...
> **⚠️ Titanis ccache note:** `-TicketCache` only works with Titanis-native ccache files; it silently provides no auth context for impacket or Metasploit-generated ccache files. `-Tgt` also rejects externally generated ccache files. To use Kerberos with Titanis, obtain a native TGT via `Kerb asreq -TicketCache` and reference it with `-Tgt`, or use `KRB5CCNAME` + `wmiexec.py` / `smbclient.py` directly.
...
> **Titanis status:** No ticket forging. Use Metasploit `auxiliary/admin/kerberos/forge_ticket` (native Ruby) or impacket `ticketer.py`. **⚠️ Forged tickets cannot be loaded into Titanis.** Use `KRB5CCNAME` + impacket (`wmiexec.py -k`, `smbclient.py -k`) directly with forged tickets.
